...
// Fila de pos-its reutilizable
function Posits({ navigate, rutaActual }) {
  return (
    <>
      {secciones.map((s, i) => {
...
className={`${colores[i % colores.length]} text-gray-800 text-[11px] font-semibold px-2 py-1.5 rounded-t-md shadow-sm transition-all cursor-pointer whitespace-nowrap ${activo ? 'opacity-0 pointer-events-none' : ''}`}          >
            {s.corto}
          </button>
        )
      })}
...
function Cuaderno({ izquierda, derecha }) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-200 via-pink-200 to-sky-200 p-2 sm:p-6 flex items-center justify-center">
      <div className="w-full max-w-5xl bg-gray-800 rounded-3xl p-3 sm:p-5 shadow-2xl">
        ...
        <div className="flex sm:hidden">
          {/* Anillos en TODO el borde izquierdo */}
            ...
            {secciones.map((s, i) => {
              ...
              return (
                <button
                  key={s.nombre}
                  onClick={() => navigate(s.ruta)}
                  className={`${colores[i % colores.length]} text-gray-800 text-[8px] font-semibold px-1.5 py-1 rounded-r-md shadow-sm cursor-pointer ${activo ? 'opacity-40' : ''}`}
                >
                  {s.corto}
                </button>
              )
